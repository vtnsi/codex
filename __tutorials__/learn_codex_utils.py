...
def describe_feature_list(df:pd.DataFrame, chosen_features:list[str]):
    for feature in chosen_features:
        try:
            ...
            raise KeyError(f"Feature {feature} does not exist in dataset. Please re-examine feature list.")
...
def write_binning_file(df:pd.DataFrame, chosen_features:list[str], feature_lines:list[str], codex_dir):
