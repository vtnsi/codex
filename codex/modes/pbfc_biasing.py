...
def balanced_train(
    df: pd.DataFrame, interaction_indices: dict, extract_combination=None, output_dir=""
):
    df_selected = None
    ...
    for combination in interaction_indices:
        ...
        interaction_least_freq = min(
            combination_dict, key=lambda x: len(set(combination_dict[x]))
        )
        interaction_max_freq = max(
            combination_dict, key=lambda x: len(set(combination_dict[x]))
        )
        ...
        for interaction_other in combination_dict:
            ...
            other_ids_sampled = random.sample(
                interaction_other_ids, sample_others_max_possible
            )
            print(
                "{}: {} sampled among interaction {}. Difference of {} from target.".format(
                    combination,
                    sample_others_max_possible,
                    interaction_other,
                    sample_others_max_possible - sample_others,
                )
            )

            df_other = pd.concat(
                (df_other, df.loc[df.index.isin(other_ids_sampled)]), axis=0
            )
    ...
    print(
        "Created {} training sets for each combination (t=2).".format(
...
def skew_dataset_relative(
    ...
    df_selected = None
    df_selected_filename = None
    combo_id_selected = None
    interaction_id_selected = None
    ni = None
    ...
    if skew_level == "baseline":
        ...
        df_selected_filename = "training_skew_<{} {}>-{}_random.csv".format(
            extract_combination, interaction_max_freq, "baseline"
        )
        ...
        return (
            df_selected,
            df_selected_filename,
            combo_id_selected,
            interaction_id_selected,
        )

    print(
        "Sampling interaction of interest. Desiring xni = {} of interaction {} from actual={}".format(
            xni, interaction_max_freq, m
        )
    )
    sample_interaction_max = int(min(np.floor([xni, m])))
    print("Choosing {} samples.".format(sample_interaction_max))
    df_interaction = df.loc[df.index.isin(most_freq_interaction_ids)].sample(
        sample_interaction_max
    )
    print(
        "{}: Sampled {} among MOST FREQ interaction, {} samples. Difference of {} from original target.".format(
            extract_combination,
            len(df_interaction),
            interaction_max_freq,
            sample_interaction_max - xni,
        )
    )

    print(
        "Sampling other interactions in combination with remainder {} among {} other interactions.".format(
            ni - sample_interaction_max, i - 1
        )
    )
    ...
    for interaction_other in combination_dict:
        ...
        print(
            "{}: Sampled {} among interaction {}. Difference of {} from original target.".format(
                extract_combination,
                len(other_ids_sampled),
                interaction_other,
                sample_others_max - ((sample_interaction_max - xni) / i - 1),
            )
        )

    filename = "training_skew_<{} {}>-{}.csv".format(
        extract_combination, interaction_max_freq, skew_level
    )
    ...
    print(
        "Total num in skewed DF towards {} {}:".format(
            extract_combination, interaction_max_freq
        ),
        df_skewed.shape,
    )
    print()
