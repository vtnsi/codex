...
def ovr_ps_perf(
    ...
    perf_dict = {
        ...
        "test": {"Overall Performance": {}, "Per-Sample Performance": {}},
    }
    ...
    X_test_data, Y_test_data, scaler_after = data.format_df_nd_all(
    ...
    avg_mode = "binary"
    perf_dict["test"]["Overall Performance"]["accuracy"] = accuracy_score(
        Y_test_data, Y_pred
    )
    ...
    perf_dict["test"]["Overall Performance"]["f1"] = (
        f1  # f1_score(Y_test_data, Y_pred, average=avg_mode)
    )
    perf_dict["test"]["Overall Performance"]["report"] = classification_report(
        Y_test_data, Y_pred
    )
    ...
    data.output_json_readable(
        perf_dict, write_json=True, file_path=os.path.join(output_dir, filename)
    )
...
def model_suite(
    ...
    X_train_data, Y_train_data, scaler_after = data.format_df_nd_all(
    ...
    if notrain:
        ...
        model_knn = None
        model_svm = None
        psp_lr, perf_filename_lr = ovr_ps_perf(
            model_lr,
            X_test,
            Y_test,
            experiment_name,
        ...
        psp_gnb, perf_filename_gnb = ovr_ps_perf(
            model_gnb,
            X_test,
            Y_test,
            experiment_name,
        ...
        psp_rf, perf_filename_rf = ovr_ps_perf(
            model_rf,
            X_test,
            Y_test,
            experiment_name,
        ...
        psp_rf, perf_filename_kn = ovr_ps_perf(
            model_knn,
            X_test,
            Y_test,
            experiment_name,
        ...
        psp_svm, perf_filename_svm = ovr_ps_perf(
            model_svm,
            X_test,
            Y_test,
            experiment_name,
...
def model_suite_parameters(
    ...
    output.output_json_readable(
        gs_lr.best_params_, write_json=True, file_path="gs_lr_{}.json".format(tag)
    )
    ...
    output.output_json_readable(
        gs_gnb.best_params_, write_json=True, file_path="gs_gnb_{}.json".format(tag)
    )
    ...
    output.output_json_readable(
        gs_rf.best_params_, write_json=True, file_path="gs_rf_{}.json".format(tag)
    )
    ...
    gs_knn = GridSearchCV(
        estimator=KNN, param_grid=params_knn, cv=2, scoring="precision"
    )
    gs_knn.fit(X_train, Y_train)
    print(gs_knn.best_params_)
    results["knn"] = gs_knn.best_params_
    output.output_json_readable(
        gs_knn.best_params_, write_json=True, file_path="gs_knn_{}.json".format(tag)
    )
    ...
    gs_svm = GridSearchCV(
        estimator=SVM, param_grid=params_svm, cv=2, scoring="precision"
    )
    gs_svm.fit(X_train, Y_train)
    print(gs_svm.best_params_)
    output.output_json_readable(
        gs_svm.best_params_, write_json=True, file_path="gs_svm_{}.json".format(tag)
    )
...
def main():
    ...
    entire_df_cont = pd.read_csv(
        os.path.join(
            data_dir_og, "diabetes_binary_health_indicators_BRFSS2015-idxd-dt_int.csv"
        )
    )
    entire_df_cont = pd.read_csv(
        os.path.join(
            data_dir_og,
            "diabetes_binary_5050split_health_indicators_BRFSS2015-idxd-dt_int.csv",
        )
    )
    ...
    trainpool_df_STATIC_1211 = pd.read_csv(
        os.path.join(data_dir_bt_STATIC_1211, "trainpool.csv"), index_col="ID"
    )
    test_df_STATIC_1211 = pd.read_csv(
        os.path.join(data_dir_bt_STATIC_1211, "test.csv"), index_col="ID"
    )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    # 250k
    train_df = entire_df_cont.loc[
        entire_df_cont.index.isin(trainpool_df_STATIC_1211.index.tolist())
    ]
    ...
    if not random:
        X_train, X_test, Y_train, Y_test, split_filename = data.prep_split_data(
            None,
            train_df=train_df,
            test_df=test_df,
            name="params",
        ...
        X_train, X_test, Y_train, Y_test = train_test_split(
            entire_cont_sampled_proc.values,
            entire_cont_sampled_proc["Diabetes_binary"].values,
            test_size=0.2,
        )
