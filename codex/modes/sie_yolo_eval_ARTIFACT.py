...
def train_SIE(
    ...
    for SIE_id in SIE_splits:
        ...
        dataset_file = os.path.join(data_config_dir, "{}incl_data.yml".format(SIE_id))
...
def get_image_paths(interaction_id, data_config_dir):
    ...
    if interaction_id != "_x_":
        with open(os.path.join(data_config_dir, interaction_id + "test_excl.txt")) as f:
...
def evaluate(
    ...
    for i, SIE_id in enumerate(SIE_splits):
        ...
        test_in_img_paths, test_ex_img_paths = get_image_paths(SIE_id, data_config_dir)
        if verbose:
