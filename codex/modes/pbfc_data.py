...
def prep_split_data(df:pd.DataFrame, target_col:str, id_col:str, split_dir, name, drop_list=[], random=False, **kwargs):
    ...
    assert id_col not in X_train.columns.tolist() and target_col not in X_train.columns.tolist()
    
    return X_train, X_test, Y_train, Y_test, split_filename

def prep_split_data_retain(df:pd.DataFrame, target_col:str, id_col:str, split_dir, name, drop_list=[], random=False, **kwargs):
    train_df = kwargs['train_df']
    test_df = kwargs['test_df']
...
def prep_data(df:pd.DataFrame, target_col:str, id_col:str, drop_list:list=[], preserve_ids=False):
    if preserve_ids:
...
def write_split(split_dir, name, train_ids, test_ids,):
    split = {'split_id': name,
             'train': train_ids,
             'test': test_ids}
    ...
    output_json_readable(split, write_json=True, file_path=os.path.join(split_dir, split_filename))

    return split, split_filename

def format_df_nd_all(X_df:pd.DataFrame, Y_ser:pd.Series, scaler:sklearn.preprocessing.StandardScaler=None):
    if scaler is None:
        X_nd = X_df.values
    else:
