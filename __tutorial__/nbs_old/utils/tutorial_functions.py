...
def __assembly_info_user__(path=None):
    ...
    print(os.getcwd())
    print("TUTORIAL MODE MATERIALS. SELECT ITEMS FROM HERE OR PROVIDE YOUR OWN ITEMS CORRECTLY: ", os.listdir(path))


def dsc_all(input_comparison):
    SIE_perf = glob.glob(os.path.join('tutorial_materials',
                         'performance', 'performance_*_*.json'))
    print(SIE_perf)
    split_ids = []
    ...
    for id in split_ids:
        ...
        input_comparison['split_file'] = ['model{}incl.json'.format(
            id), 'model{}excl.json'.format(id), 'model{}all.json'.format(id), 'model_x_all.json']
        input_comparison['performance_file'] = ['performance{}incl.json'.format(
            id), 'performance{}excl.json'.format(id), 'performance{}all.json'.format(id), 'performance_x_all.json']
...
def assemble_input_file_user(codex_input_demo):
    ...
    modes = ['dataset evaluation', 'dataset split evaluation',
             'dataset split comparison', 'performance by interaction', 'model probing']
    ...
    codex_dir = input("CODEX DIRECTORY: (default: {})".format(
        codex_input_demo['codex_directory']))
    if codex_dir != '':
    ...
    print("Selected {} as experiment config ID.".format(
        codex_input_demo['config_id']))
    ...
    codex_input_demo['features'] = input(
        "FEATURES (separate by commas, ','):").strip(' ').split(',')
    ...
    codex_input_demo['timed_output'] = (
        input("TIMESTAMP OUTPUT FOLDER (y/n): ") == 'y')
...
def probe_splits():
    ...
    split_99 = {'train': split_00['train'], 'validation': split_00['validation'],
                'test_probe': probe_ids, 'test_exploit': exploit_ids}
    output.output_json_readable(split_99, write_json=True, file_path=os.path.join(
        'tutorial_materials/splits/split_99.json'))
    ...
    performance_99['test_probe']['Overall Performance']['accuracy'] = np.mean(
        [performance_99['test_probe']['Per Sample Performance'][id]['accuracy'] for id in performance_99['test_probe']['Per Sample Performance']])
    performance_99['test_exploit']['Overall Performance']['accuracy'] = np.mean(
        [performance_99['test_exploit']['Per Sample Performance'][id]['accuracy'] for id in performance_99['test_exploit']['Per Sample Performance']])

    output.output_json_readable(performance_99, write_json=True,
...
def display_results(results_dict, element, strengths, **kwargs):
    ...
    if element == 'SDCC':
        ...
        assert sdcc_element in ['train', 'validation', 'test',
                                'train-test', 'test-train', 'validation-test', 'test-validation']
        # call
    else:
        pass
